...
| Обработка Telegram | Обработчики текста, команд и фото; `chat_id`, текст, вложения. |
...
**Зависимости:** обработчики используют хранилища диалогов и транзакций и слой LLM; слой LLM не импортирует Telegram.

**Инфраструктура:** один процесс; внешние вызовы — Telegram API и настроенный **LLM backend** (облако или локальный Ollama и т.п.).
...
Групповые чаты и голос — **вне скоупа MVP**, если не появится отдельное требование. **Фото (чеки)** — **в скоупе** после соответствующей итерации.
