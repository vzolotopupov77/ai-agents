...
Бот ведёт **диалог** с моделью (OpenRouter через OpenAI-совместимый клиент) и по плану развития — **учёт доходов и расходов** в памяти процесса. Роль и тон задаются **`SYSTEM_PROMPT`** в `.env`; пример переменных — [.env.example](.env.example).
...
- **Текстовые сообщения** — ответ модели в роли советника; история диалога в памяти с лимитом `MAX_HISTORY_MESSAGES`.

**Пока не реализовано** (следующие итерации): автоматическое извлечение транзакций из текста (structured output), обработка фото чеков (VLM), единый префикс переменных `LLM_*` вместо `OPENROUTER_*` — см. tasklist.
...
2. Переменные окружения: скопируйте [.env.example](.env.example) в `.env` и заполните значения (подробнее в vision, раздел «Конфигурация»).
