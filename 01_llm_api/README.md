...
Суффикс `:free` у конкретных моделей на OpenRouter часто меняется; `openrouter/free` стабильнее для учебных запусков без списания кредитов. Платный вариант при балансе: `openai/gpt-4o-mini` (без `:free`).
...
Примеры бесплатных моделей (актуальный список — [коллекция Free](https://openrouter.ai/collections/free-models)):
- `openrouter/free` — роутер, выбирает доступную free-модель
- `openai/gpt-oss-20b:free`, `openai/gpt-oss-120b:free`
- Другие `:free` ID смотрите в каталоге; старые вроде `openai/gpt-4o-mini:free` могут давать 404
