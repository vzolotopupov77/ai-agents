...
| 7 | 22 | Инструмент `calculate_deposit_profit` в MCP сервере | ✅ Завершено | 22.05.2026 |
| 7 | 23 | Системный промпт и помощь `/help` под новый инструмент | ✅ Завершено | 22.05.2026 |
| 7 | 24 | Тестирование, README MCP сервера, документация | ✅ Завершено | 22.05.2026 |

> **ДЗ-08** (домашнее задание модуля): **ДЗ-08-1** — проверка MCP после Спринта 6; **ДЗ-08-2** — реализация и сдача **Спринта 7** (итерации 22–24, `calculate_deposit_profit`).
...
- [x] Обновить README.md
  - [x] Описать новые возможности с MCP (4 инструмента с rag_search)
...
- [x] Обновить /help команду бота
  - [x] Добавить описание MCP-инструментов (в т.ч. calculate_deposit_profit после спринта 7)
...
## 🚀 Спринт 7: Инструмент расчёта доходности вклада (ДЗ-08, часть 2)

Реализация домашнего задания **ДЗ-08-2**: новый MCP-инструмент, промпт агента, E2E и документация.

### Итерация 22: Реализация `calculate_deposit_profit` в MCP сервере

**Цель:** Добавить вычислительный инструмент вклада в `mcp/mcp-bank-agent/server.py`

- [x] Функция `compute_deposit_profit(initial_amount, annual_rate, term_months, capitalization)` — чистая математика
- [x] Без капитализации: доход = сумма × (ставка/100/12) × срок в месяцах; итог = сумма + доход
- [x] С капитализацией (ежемесячная): итог по `S * (1 + r/12/100)^n`, где `n` — месяцы; доход = итог − `S`
- [x] `@mcp.tool` `calculate_deposit_profit` с `Annotated` и `Field` (границы аргументов как в контракте)
- [x] Возврат пользователю: читаемая строка с доходом, итогом и disclaimer (не оферта, упрощённая модель)

**Как протестировать:**
- Вызов инструмента через MCP клиент или через бота (после итерации 23–24): оба режима `capitalization` и граничные параметры из `Field`

---

### Итерация 23: Обновление системного промпта агента

**Цель:** Чтобы MCP client (агент) стабильно выбирал расчёт при запросах на «сколько заработаю на вкладе»

- [x] В `prompts/agent_system.txt`: пункт 4 в разделе ИНСТРУМЕНТЫ, разница с остальными инструментами
- [x] Секция КОГДА ИСПОЛЬЗОВАТЬ `calculate_deposit_profit` с примерами вызова (`initial_amount`, `annual_rate`, `term_months`, `capitalization`)
- [x] Команда `/help` (`handlers.py`): новый пункт в списке инструментов и примеры вопросов

**Как протестировать:**
- Диалоги «100 000 под 15% на 12 месяцев с/без капитализации» — агент вызывает `calculate_deposit_profit`

---

### Итерация 24: Тестирование и финальная полировка

**Цель:** E2E, документация MCP и синхронизация с общим описанием проекта

- [x] Обновить `mcp/mcp-bank-agent/README.md` — описание инструмента, примеры, разделение ответственности
- [x] Обновить `docs/idea.md`, `docs/vision.md`, `docs/tasklist.md`
- [x] Комментарии в `src/agent.py` под четыре типа инструментов

**Как протестировать:**
- `make run-mcp-bank` + `make run`, сценарии из README по `calculate_deposit_profit`

---

## 🏠 ДЗ-08: MCP — домашнее задание модуля 08-mcp

Домашнее задание состоит из двух шагов и **включает Спринт 7**: сначала проверка готового MCP (Спринт 6), затем доработка по **Спринту 7**.

| Шаг | Спринт | Содержание | Статус |
|-----|--------|------------|--------|
| ДЗ-08-1 | 6 (проверка) | Запуск MCP и бота, `search_products`, `currency_converter`, LangSmith | ✅ |
| ДЗ-08-2 | **7** (ит. 22–24) | `calculate_deposit_profit`, промпт, `/help`, E2E, документация | ✅ |

### ДЗ-08-1: Запуск MCP сервера и проверка инструментов

**Цель:** Убедиться, что стек после **Спринта 6** работает (база для Спринта 7).

- [x] Настроить `.env` и установить зависимости (`make install`)
- [x] Запустить MCP сервер: `make run-mcp-bank`
- [x] Запустить бота: `make run`
- [x] Проверить `search_products`: вопросы об актуальных ставках по вкладам
- [x] Проверить `currency_converter`: курс доллара/евро
- [x] Посмотреть трейсы MCP-инструментов в LangSmith

### ДЗ-08-2: Новый инструмент и Спринт 7

**Цель:** Выполнить **Спринт 7** (итерации 22–24) — см. раздел выше.

- [x] **Ит. 22:** `@mcp.tool()` `calculate_deposit_profit` в `mcp/mcp-bank-agent/server.py`
- [x] **Ит. 23:** `prompts/agent_system.txt`, команда `/help`
- [x] **Ит. 24:** E2E через бота, README и `docs/*`
- [x] Перезапуск MCP + бота; трейс `calculate_deposit_profit` в LangSmith
