...
## 🏠 ДЗ-08: MCP — домашние шаги

| Итерация | Функционал | Статус |
|----------|------------|--------|
| ДЗ-08-1 | Запуск MCP сервера и проверка инструментов | ⏳ |
| ДЗ-08-2 | Новый инструмент в MCP сервере | ⏳ |

### ДЗ-08-1: Запуск MCP сервера и проверка инструментов

**Цель:** Запустить готовый MCP сервер, проверить работу инструментов через бота

- [ ] Настроить `.env` и установить зависимости (`make install`)
- [ ] Запустить MCP сервер: `make run-mcp-bank`
- [ ] Запустить бота: `make run`
- [ ] Проверить `search_products`: задать вопросы об актуальных ставках по вкладам
- [ ] Проверить `currency_converter`: задать вопрос о курсе доллара/евро
- [ ] Посмотреть трейсы вызовов MCP инструментов в LangSmith

### ДЗ-08-2: Новый инструмент в MCP сервере

**Цель:** Добавить свой инструмент в `mcp/mcp-bank-agent/server.py`

- [ ] Добавить новый инструмент с `@mcp.tool()` (например, `calculate_deposit_profit` или свой)
- [ ] Перезапустить MCP сервер и бота
- [ ] Убедиться что агент вызывает новый инструмент
- [ ] Проверить трейс нового инструмента в LangSmith
