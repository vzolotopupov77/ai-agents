...
async def cmd_start(message: Message):
    ...
    await message.answer(
        "Привет! Я ReAct-агент ассистента Сбербанка.\n\n"
        "Я могу:\n"
        "• Отвечать на вопросы по документам банка (кредиты, вклады и др.)\n"
        "• Считать курс и перевод суммы между валютами (приоритет — ЦБ РФ)\n"
        "• Вести диалог с учётом контекста\n\n"
        "Подробности и команды — /help."
    )
...
async def cmd_help(message: Message):
    ...
    help_text = (
        ...
        "Я интеллектуальный агент\\, который *сам выбирает*\\, когда искать в документах и когда считать курс валют\\.\n\n"
        "*🧠 Как я работаю \\(ReAct\\):*\n"
        "1\\. *Reason* \\- анализирую вопрос и историю диалога\n"
        "2\\. *Act* \\- при необходимости вызываю инструменты:\n"
        "• `rag_search` \\- ответы по документам банка \\(кредиты\\, вклады\\.\\.\\.\\)\n"
        "• `currency_converter` \\- конвертация валют \\(сначала курс ЦБ РФ\\; при недоступности сайта ЦБ \\- запасной источник с пометкой в ответе\\)\n"
        "3\\. *Respond* \\- формирую ответ на русском\n\n"
        "*📋 Доступные команды:*\n"
        ...
        "*✨ Возможности:*\n"
        "• Два инструмента агента: документы и валюты\n"
        "• История диалога \\(MemorySaver\\)\n"
        "• Гибридный поиск в трёх режимах\n"
        "• RAGAS метрики качества\n"
        "• LangSmith трейсинг \\(настройка через переменные в \\.env\\)\n\n"
        "*💬 Примеры вопросов:*\n"
        "• Какие условия потребительского кредита?\n"
        "• Какие проценты по вкладам?\n"
        "• Можно ли досрочно погасить кредит?\n"
        "• Сколько будет 500 долларов в рублях?\n"
        "• Привет \\(ответ без инструментов\\)\n\n"
        "_Используй /index\\_status для текущей конфигурации\\._"
    )
...
async def handle_message(message: Message):
    ...
    try:
        ...
        logger.warning(
            "Rate limit (429) для chat %s — см. MODEL в .env и лимиты OpenRouter",
            message.chat.id,
        )
        await message.answer(
            "Провайдер модели временно ограничил запросы (часто у бесплатных моделей на OpenRouter).\n\n"
            "Что можно сделать: подождать 1–2 минуты и повторить; увеличить LLM_MAX_RETRIES в .env; "
            "сменить MODEL на другую модель или добавить свой ключ: "
