...
| ДЗ-7 | 1 | Собственный инструмент агента | ⏳ Не начато | — |
...
> Эталонный проект уже содержит ReAct-агента с одним инструментом `rag_search`.
> Эти итерации — для домашней работы студента: добавить второй tool и провести наблюдения.
> Имена итераций — с префиксом `ДЗ-7:`, чтобы не смешиваться с учебными спринтами выше.

### ДЗ-7: Итерация 1 — собственный инструмент агента

**Цель:** Добавить второй `@tool` к агенту и убедиться, что он корректно используется в ReAct-цикле.

- [ ] Выбрать инструмент: `currency_converter` (заглушка с фиксированными курсами или реальный API) либо свой вариант (погода, калькулятор, поиск отделений и т.п.).
- [ ] Реализовать функцию в `src/tools.py` под `@tool` с аккуратной docstring (LLM использует её как описание).
- [ ] Подключить инструмент в `src/agent.py` — добавить в список `tools` при создании агента.
- [ ] Дополнить `prompts/agent_system.txt`: описать, когда звать новый инструмент, добавить 1–2 few-shot примера.
- [ ] Перезапустить бота и проверить вызов инструмента в LangSmith Threads.
...
**Цель:** Проверить поведение агента и подготовить отчёт для Telegram.

- [ ] Прогнать сценарии: простая реплика (без `rag_search`), продуктовый вопрос (с `rag_search`), вопрос по новому инструменту.
- [ ] Изучить трейсы в LangSmith → Threads: на каких вопросах какие инструменты звались.
- [ ] Сделать скриншот: либо переписка в Telegram с вызовом инструмента, либо Thread в LangSmith.

**Как протестировать:**
- В трейсе видно последовательность Reason → Act(<tool>) → Respond.
- Скриншот сохранён локально для прикрепления к Telegram-сообщению.
