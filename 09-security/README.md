...
- 🛠️ **6 типов инструментов:**
  ...
  - `open_deposit` - открытие вклада с Human-in-the-Loop подтверждением (MCP)
...
- 🎯 **Умная стратегия:**
  ...
  - Открытие карт / вкладов → open_credit_card / open_deposit (HITL, MCP)
- 📝 **Гибкие промпты** - системный промпт загружается из файла
- ⚙️ **Лимиты цикла ReAct** — не более заданного числа вызовов LLM и инструментов за одно сообщение (`ModelCallLimitMiddleware`, `ToolCallLimitMiddleware` в `middleware.py`)
...
- 💳 **Открытие карт** - `open_credit_card` требует явного одобрения пользователя
- 🏦 **Открытие вкладов** - `open_deposit` требует явного одобрения пользователя
- ✅ **Inline кнопки** - удобный интерфейс Approve/Reject
- 🔄 **Отклонение с причиной** - агент корректно обрабатывает отказы
- 🎯 **Middleware архитектура** - легко добавить HITL к любому инструменту (одна строка в `interrupt_on`)
- 🔐 **Маскирование номера карты в ответе** — `PIIMiddleware` (`src/middleware.py`): в Telegram уходит только маска вида `****-****-****-XXXX`
...
**Открытие вклада с Human-in-the-Loop (open_deposit via MCP):**
```
👤 Хочу открыть вклад

🤖 Хорошо, помогу открыть вклад. Уточните, пожалуйста:
   • Ваше имя (как в паспорте)?
   • Сумму вклада?
   • Процентную ставку?
   • Срок вклада (в месяцах)?

👤 Иван Петров, 200 тысяч под 15% на 12 месяцев

🤖 ⚠️ Требуется подтверждение операции
   
   🔧 Инструмент: open_deposit
   📋 Параметры:
...
🤖 ✅ Вклад успешно открыт!
   ...
   📋 Детали вклада:
      ...
      Сумма: 200,000₽
      Процентная ставка: 15.0% годовых
      Срок: 12 мес.
   
   💰 Ожидаемый доход:
      Доход: 30,000.00₽
      Итоговая сумма: 230,000.00₽
```
...
- `open_credit_card` - открытие карты (требует HITL подтверждения)
- `open_deposit` - открытие вклада (требует HITL подтверждения)
...
2. **currency_converter** - конвертация валют по курсам ЦБ РФ
   - Источники (fallback): `cbr-xml-daily.ru` → `open.er-api.com`
...
4. **open_credit_card** - открытие дебетовых и кредитных карт
   ...
   - Безопасность: CVV не возвращается, PAN маскируется (`PIIMiddleware`)

5. **open_deposit** - открытие вклада
   - **Human-in-the-Loop** - требует подтверждения пользователя
   - Параметры: имя клиента, сумма, срок, ставка
   - Возвращает: номер договора и ожидаемый доход
