...
3. **agent.py** - ReAct агент с MCP интеграцией и Guard Middleware
   - `create_bank_agent()` - создание агента через `create_agent()` из LangChain 1.0
   - **Guard Middleware Stack (4 уровня):**
     - Layer 1: ModelCallLimitMiddleware - ограничение вызовов модели (защита от runaway)
     - Layer 2: ToolCallLimitMiddleware - ограничение вызовов инструментов (защита от tool bombing)
     - Layer 3: PIIMiddleware - маскирование credit_card номеров в ответах (apply_to_output=True)
     - Layer 4: HumanInTheLoopMiddleware - подтверждение для open_credit_card и open_deposit
   - Настройка `interrupt_on` для финансовых операций (легко расширяется)
   ...
   - Логирование конфигурации middleware при инициализации
...
6. **config.py** - конфигурация
   ...
   - Guard Middleware настройки: `GUARD_PII_CREDIT_CARD_ENABLED`, `GUARD_PII_CREDIT_CARD_STRATEGY`, `GUARD_MODEL_CALL_LIMIT`, `GUARD_TOOL_CALL_LIMIT`, `GUARD_LIMITS_EXIT_BEHAVIOR`
...
9. **mcp/mcp-bank-agent/** - MCP сервер для банковских инструментов
   ...
   - `open_deposit(deposit_name, amount, term_months)` - открытие депозитного счета (требует HITL)
...
bank_agent (ReAct цикл с 5 типами инструментов):
    ...
    2. Act - вызов одного из инструментов:
       ...
       │
       ├─ MCP::open_deposit(deposit_name, amount, term_months) → открытие вклада
       │  ⚠️ INTERRUPT! HumanInTheLoopMiddleware останавливает выполнение
       │  → возврат interrupt объекта в handlers.py через agent_answer()
       │  → handlers.py форматирует сообщение: "💰 Открытие вклада..."
       │  → показ кнопок "✅ Подтвердить" / "❌ Отклонить" в Telegram
       ...
       │  IF approve:
       │    → HTTP запрос к mcp-bank-agent (port 8000)
       │    → MCP сервер выполняет open_deposit
       │    → генерация номера счета, расчет даты окончания, прогноз дохода
       │    → возврат данных вклада (номер счета, условия, прогноз)
       │    → агент формирует ответ с данными вклада
       │  IF reject:
...
**Цель:** Автоматизированная проверка качества работы ReAct агента через траекторную оценку (trajectory evaluation).

**Траектория агента** - последовательность действий:
- Входящие сообщения пользователя
- Вызовы инструментов с аргументами
- Результаты инструментов
- Финальные ответы агента

**Технологии:**
- `pytest` - фреймворк для тестирования
- `pytest-asyncio` - поддержка асинхронных тестов
- `agentevals` - библиотека для траекторной оценки агентов
- `MemorySaver` - изоляция тестовых сессий агента
...
Проверяет что агент вызывает правильные инструменты с правильными аргументами.

**Режимы:**
- **superset** - агент вызывает минимум требуемые инструменты (может делать дополнительные)
- **subset** - агент НЕ вызывает лишние инструменты
- **strict** - точное совпадение траектории
...
- `agent_fixture` - создание агента с MemorySaver
- `mcp_server_fixture` - запуск MCP сервера для тестов (опционально)
- Уникальные `thread_id` для каждого теста (изоляция состояния)

**Изоляция тестов:**
- Каждый тест использует уникальный `thread_id` (например: "test_rag_1", "test_mcp_2")
- MemorySaver хранит состояние отдельно для каждого `thread_id`
...
HITL (Human-in-the-Loop) тесты проверяют что агент корректно останавливается для подтверждения и продолжает выполнение после решения пользователя.

**Как работает:**
1. Запустить агента с запросом на критичную операцию (например "Открой вклад")
2. Агент доходит до `open_deposit` → interrupt
3. Программно отправить "approve" (эмулируя пользователя)
4. Агент продолжает выполнение и завершает операцию
5. Извлечь полную траекторию (включая interrupt + resume)
6. LLM-as-judge оценивает разумность траектории

**Это НЕ моки** - агент реально работает, interrupt происходит через middleware, мы просто автоматически подтверждаем действие в тесте.
...
- Корректность аргументов инструментов
- Обработка HITL флоу (interrupt → approve/reject)
- Многошаговые диалоги с комбинацией инструментов

**Что НЕ тестируем:**
- Юнит-тесты отдельных функций (избыточно для MVP)
- Интеграционные тесты MCP сервера (проверяется в e2e)
- UI тесты Telegram бота (проверяется вручную)
