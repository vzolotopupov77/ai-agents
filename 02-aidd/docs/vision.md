...
1. **`system`** — текст системного промпта из переменной окружения (текущая роль — коуч карьерного развития; см. [idea.md](./idea.md) и §8).
...
| **Старт** | По команде `/start`: короткое приветствие без вызова LLM; контекстуализировано под роль коуча (реализовано в итерации 3); историю не очищает. |
...
| `SYSTEM_PROMPT` | Системный промпт (роль ассистента). **Текущая роль:** коуч карьерного развития — помогает найти работу, подготовиться к интервью, составить резюме и сопроводительное письмо. Многострочность — в кавычках или экранирование по правилам `.env`. |
